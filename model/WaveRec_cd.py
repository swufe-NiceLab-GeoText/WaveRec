...
class DWT2D(nn.Module):
    def __init__(self, wave: str = 'haar'):
        ...
        if wave == 'haar' or wave == 'db1':
            ...
            self.dec_lo = torch.tensor([0.4829629131445341, 0.8365163037378079, 0.2241438680420134, -0.1294095225512604], dtype=torch.float32)
            self.dec_hi = torch.tensor([-0.1294095225512604, -0.2241438680420134, 0.8365163037378079, -0.4829629131445341], dtype=torch.float32)
        elif wave == 'db4':
...
class IDWT2D(nn.Module):
    def __init__(self, wave: str = 'haar'):
        ...
        if wave == 'haar' or wave == 'db1':
            ...
            self.rec_lo = torch.tensor([-0.1294095225512604, 0.2241438680420134, 0.8365163037378079, 0.4829629131445341], dtype=torch.float32)
            self.rec_hi = torch.tensor([-0.4829629131445341, 0.8365163037378079, -0.2241438680420134, -0.1294095225512604], dtype=torch.float32)
        elif wave == 'db4':
    ...
    def forward(self, LL: torch.Tensor, LH: torch.Tensor, HL: torch.Tensor, HH: torch.Tensor) -> torch.Tensor:
        ...
        lo_col = F.conv_transpose2d(LL, rec_lo.expand(C, 1, 1, -1), groups=C, stride=(1, 2), padding=(0, self.filter_len // 2))
        lo_col = lo_col + F.conv_transpose2d(LH, rec_hi.expand(C, 1, 1, -1), groups=C, stride=(1, 2), padding=(0, self.filter_len // 2))

        hi_col = F.conv_transpose2d(HL, rec_lo.expand(C, 1, 1, -1), groups=C, stride=(1, 2), padding=(0, self.filter_len // 2))
        hi_col = hi_col + F.conv_transpose2d(HH, rec_hi.expand(C, 1, 1, -1), groups=C, stride=(1, 2), padding=(0, self.filter_len // 2))

        rec_lo_t = self.rec_lo.to(device).to(dtype).view(1, 1, -1, 1)
        rec_hi_t = self.rec_hi.to(device).to(dtype).view(1, 1, -1, 1)

        output = F.conv_transpose2d(lo_col, rec_lo_t.expand(C, 1, -1, 1), groups=C, stride=(2, 1), padding=(self.filter_len // 2, 0))
        output = output + F.conv_transpose2d(hi_col, rec_hi_t.expand(C, 1, -1, 1), groups=C, stride=(2, 1), padding=(self.filter_len // 2, 0))
...
class ExternalContextEncoder(nn.Module):
    def __init__(self, ext_dim: int = 10, hidden_dim: int = 64, num_heads: int = 4, num_layers: int = 2, dropout: float = 0.1):
        super(ExternalContextEncoder, self).__init__()

        self.input_proj = nn.Linear(ext_dim, hidden_dim)
    ...
    def forward(self, ext_features: torch.Tensor) -> torch.Tensor:
        x = self.input_proj(ext_features)
        x = x.unsqueeze(1)
...
class CrossModalAttention(nn.Module):
    def __init__(self, feat_channels: int, ext_dim: int, num_heads: int = 4, dropout: float = 0.1):
        ...
        self.q_proj = nn.Conv2d(feat_channels, feat_channels, kernel_size=1)
        self.k_proj = nn.Linear(ext_dim, feat_channels)
        self.v_proj = nn.Linear(ext_dim, feat_channels)

        self.out_proj = nn.Conv2d(feat_channels, feat_channels, kernel_size=1)
    ...
    def forward(self, low_freq_feat: torch.Tensor, ext_context: torch.Tensor) -> torch.Tensor:
        B, C, H, W = low_freq_feat.shape

        Q = self.q_proj(low_freq_feat).view(B, C, H * W).permute(0, 2, 1)
        K = self.k_proj(ext_context).unsqueeze(1)
        V = self.v_proj(ext_context).unsqueeze(1)
        ...
        residual = low_freq_feat + out

        residual_flat = residual.view(B, C, H * W).transpose(-1, -2)
        residual_flat = residual_flat + self.ffn(self.norm2(residual_flat))
        out = residual_flat.transpose(-1, -2).view(B, C, H, W)

        return out


class ExternalContextFusion(nn.Module):
    def __init__(self, channels: int, ext_dim: int = 10, hidden_dim: int = 64,
                 num_heads: int = 4, num_layers: int = 2, dropout: float = 0.1):
        super(ExternalContextFusion, self).__init__()

        self.ext_encoder = ExternalContextEncoder(ext_dim, hidden_dim, num_heads, num_layers, dropout)

        self.ext_proj = nn.Linear(hidden_dim, channels)

        self.cross_attn = CrossModalAttention(channels, channels, num_heads, dropout)
    ...
    def forward(self, low_freq_feat: torch.Tensor, ext_features: torch.Tensor) -> torch.Tensor:
        ext_context = self.ext_encoder(ext_features)
        ext_context_proj = self.ext_proj(ext_context)

        fused_feat = self.cross_attn(low_freq_feat, ext_context_proj)

        refined_feat = self.refine_conv(fused_feat)
        channel_attn = self.channel_attention(refined_feat)
        refined_feat = fused_feat + refined_feat * channel_attn

        gate = self.fusion_gate(torch.cat([low_freq_feat, refined_feat], dim=1))
        output = low_freq_feat * (1 - gate) + refined_feat * gate

        return output


class RoadStructureEncoder(nn.Module):
    def __init__(self, in_channels: int = 1, hidden_dim: int = 64):
        super(RoadStructureEncoder, self).__init__()

        self.multi_scale_conv = nn.ModuleList([
            nn.Sequential(
                nn.Conv2d(in_channels, hidden_dim, kernel_size=k, padding=k//2),
    ...
    def forward(self, road_map: torch.Tensor) -> torch.Tensor:
        multi_scale_feats = [conv(road_map) for conv in self.multi_scale_conv]
        multi_scale_fused = self.fusion(torch.cat(multi_scale_feats, dim=1))

        return multi_scale_fused
...
class SpatialStructureAttention(nn.Module):
    ...
    def forward(self, high_freq_feat: torch.Tensor, road_feat: torch.Tensor) -> torch.Tensor:
        B, C, H, W = high_freq_feat.shape

        Q = self.query_conv(high_freq_feat).view(B, self.num_heads, self.head_dim, H * W).permute(0, 1, 3, 2)
        K = self.key_conv(road_feat).view(B, self.num_heads, self.head_dim, H * W).permute(0, 1, 3, 2)
        V = self.value_conv(road_feat).view(B, self.num_heads, self.head_dim, H * W).permute(0, 1, 3, 2)
        ...
        gate = self.spatial_gate(torch.cat([high_freq_feat, out], dim=1))
        output = high_freq_feat + out * gate

        return output


class RoadStructureFusion(nn.Module):
    def __init__(self, channels: int, road_channels: int = 64, num_heads: int = 4, dropout: float = 0.1):
        super(RoadStructureFusion, self).__init__()

        self.road_encoder = RoadStructureEncoder(in_channels=1, hidden_dim=road_channels)

        self.road_proj = nn.Conv2d(road_channels, channels, kernel_size=1)

        self.spatial_attn = SpatialStructureAttention(channels, num_heads, dropout)
    ...
    def forward(self, high_freq_feat: torch.Tensor, road_map: torch.Tensor) -> torch.Tensor:
        road_feat = self.road_encoder(road_map)
        road_feat = self.road_proj(road_feat)

        if road_feat.shape[-2:] != high_freq_feat.shape[-2:]:
            road_feat = F.interpolate(road_feat, size=high_freq_feat.shape[-2:], mode='bilinear', align_corners=False)

        fused_feat = self.spatial_attn(high_freq_feat, road_feat)

        edge_feat = self.edge_enhance(fused_feat)
        edge_attn = self.edge_attention(edge_feat)
        edge_enhanced = fused_feat + edge_feat * edge_attn

        refined = self.high_freq_refine(edge_enhanced)

        gate = self.fusion_gate(torch.cat([high_freq_feat, refined], dim=1))
        output = high_freq_feat * (1 - gate) + refined * gate
...
class EnhancedFPN(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for i, (feat, fpn_conv, dual_attn) in enumerate(zip(features, self.fpn_convs, self.dual_attention_modules)):
            ...
            if i > 0:
                attended_feat = F.interpolate(attended_feat, size=features[0].shape[-2:], mode='bilinear', align_corners=False)
...
class LightweightFeatureExtractorV8Pro(nn.Module):
    def __init__(self, in_channels: int = 2, out_channels: int = 64, base_channels: int = 64,
                 num_blocks: int = 2, growth_rate: int = 32, num_layers: int = 4, num_scales: int = 4,
...
class SpatialMask(nn.Module):
    def __init__(self, mask_ratio=0.70, patch_size=4):
        super().__init__()
    ...
    def forward(self, x):
        B, C, H, W = x.shape
        assert H % self.patch_size == 0 and W % self.patch_size == 0, f"{H}x{W} cannot be divided by {self.patch_size}"
...
class WaveRec(nn.Module):
    def __init__(self, height=16, width=16, use_exf=False, scale_factor=4,
                 channels=128, sub_region=4, scaler_X=1, scaler_Y=1, args=None,
                 use_wavelet: bool = True, wave: str = 'haar',
                 ext_dim: int = 10, road_channels: int = 64):
        super(WaveRec, self).__init__()
        self.height = height
        self.width = width
        self.masker = SpatialMask(mask_ratio=0.50, patch_size=4)
        ...
        if use_exf:
            ...
            self.embed_weather = nn.Embedding(15, 3)
