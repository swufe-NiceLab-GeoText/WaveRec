...
import PIL.ImageOps
from model.WaveRec_cd import WaveRec
...
save_path = f'./masked-{args.model}-{args.dataset}-{args.n_channels}-focal_l1-lightweight-CD'
os.makedirs(save_path, exist_ok=True)


class FocalL1Loss(torch.nn.Module):
    def __init__(self, beta_init=0.2, gamma_init=1.0, activate='tanh'):
...
class MaskedFocalL1Loss(torch.nn.Module):
    ...
    def forward(self, pred, target, mask=None, epsilon=1e-6):
        pred = torch.clamp(pred, min=0.0, max=1e6)
...
def calc_recon_loss(pred, target, mask, patch_size=4):
...
class MaskedTrainer:
    ...
    def _pretrain_phase(self, model, optimizer, dataloader, task_name):
        ...
        for epoch in range(self.args.pretrain_epochs):
            ...
            for c_map, f_map, exf in tqdm(dataloader, desc=f"Pretrain Epoch {epoch + 1}"):
                ...
                recon_loss = calc_recon_loss(recon, f_map, mask, patch_size=4)
    ...
    def test_task(self, model, test_loader, task_name):
        ...
        metrics = {"MSE": 0, "MAE": 0, "MAPE": 0, "RMSE": 0}
        with torch.no_grad():
